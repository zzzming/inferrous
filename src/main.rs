use anyhow::Result;
use candle_core::{Device, IndexOp, Tensor, quantized::gguf_file};
...
fn main() -> Result<()> {
    ...
    println!(
        "⚡ Auto-configured: {} threads for optimal performance",
    ...
    loop {
        ...
        for _step in 0..config.max_tokens {
            ...
            let logits =
                model_weights.forward(&input_tensor, if _step == 0 { 0 } else { start_pos })?;
